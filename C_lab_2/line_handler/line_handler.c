...
bool isIt_A_LINE(Params *params) { return (isFlagOn(&params->A)) && (params->A.counter > 0); }
...
bool isLineMatch(Params *user_params, rChar *regex_string, const char *line)
{
  ...
  bool ret_val = false;
  if (match_exact) {
    ret_val = (regexlen(regex_string) == strlenWithoutNewline(line) && isSubStrAtPlace_rec(regex_string, line));
  } else {
...
void handleLine(Params *user_params, rChar *regex_string, const char *line)
{
  ...
  if (got_match && isFlagOn(&user_params->A)) {
    if (isFlagAEndOfBlock(&user_params->A)) {
      printf("--\n");
    }
